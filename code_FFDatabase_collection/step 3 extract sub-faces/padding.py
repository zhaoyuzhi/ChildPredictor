...
def out_padding(img):
    # get original size and addition size
    h, w, c = img.shape
    if h >= w:
        t = h / 2
        h_ = t
    ...
    h_ = int(h_)
    w_ = int(w_)
    top_size, bottom_size, left_size, right_size = (int(h_), int(h_), int(w_), int(w_))
    # operation
    #replicate_padding_img = cv2.copyMakeBorder(img, top_size, bottom_size, left_size, right_size, cv2.BORDER_REPLICATE)
    img = cv2.copyMakeBorder(img, top_size, bottom_size, left_size, right_size, cv2.BORDER_CONSTANT, value = 0)
    return img
...
if __name__ == "__main__":
    ...
    # read one image
    root = 'C:\\Users\\yzzha\\Desktop\\child faces\\processed\\Asia\\Korean family\\17.jpg'
