...
def train(args, logger=None, vis_logger=None):
    ...
    for epoch in range(0, args.TRAIN.epochs):
        ...
        cos_list  = []
        l1_list   = []
        for idx, (father, mother, child, childlabel) in enumerate(bar):
            father = father.cuda()
            mother = mother.cuda()
            child  = child[0].cuda()
            ...
            cosloss = 0
            #loss = 0
            decrease_ratio_list = [0.8, 0.6, 0.3, 0.1]    #显式指定
            compare_l1loss_list = []
            this_l1loss_list = []
            this_cosloss_list = []
            ...
            for i in range(args.MODEL.mapping.out_num):
                ...
                this_l1loss_list.append(L1_loss(recon_child_code[i], child_code.detach()))
                this_cosloss_list.append(cor_square_error(recon_child_code[i], child_code.detach()))
           ...
           # loss    = args.TRAIN.l1_weight * l1loss + args.TRAIN.cos_weight * cosloss# + min_l1loss
            ...
            if args.TRAIN.inference and idx%args.TRAIN.save_iter==0:
                ...
                with torch.no_grad():       
                    recon_code = torch.cat((recon_child_code[min_index], childlabel, uniform_code), dim=1)
                    encode_code = torch.cat((child_code, childlabel, uniform_code), dim=1)
