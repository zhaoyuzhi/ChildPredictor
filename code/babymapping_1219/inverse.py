...
def train(args, logger=None, vis_logger=None):
    ...
    for epoch in range(0, args.TRAIN.epochs):
        ...
        en_vgg_loss_list = []
        encoder_loss_list = []
        for idx, input_img in enumerate(bar):
            input_img = input_img.cuda()

            latent_code, skip = encoder_net(input_img)  #输入为gt，得到其inverse后的code
            ...
            latent_loss = 1. * cor_square_error(recon_latent_code, latent_code) +\
                          1. * MSE_loss(recon_latent_code, latent_code)
