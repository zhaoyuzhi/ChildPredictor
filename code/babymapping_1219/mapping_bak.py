...
def train(args, logger=None, vis_logger=None):
    ...
    for epoch in range(0, args.TRAIN.epochs):
        ...
        cos_list  = []
        l1_list   = []
        for idx, (father, mother, child) in enumerate(bar):
            father = father.cuda()
            mother = mother.cuda()
            child  = child[0].cuda()
            ...
            for i in range(args.MODEL.mapping.out_num):
                decrease_ratio = decrease_sequence * (1 - i / args.MODEL.mapping.out_num)  #1.0 0.8 0.6 0.4 0.2
                this_l1loss_list.append(L1_loss(recon_child_code[:, (args.MODEL.mapping.out_channels * i):(args.MODEL.mapping.out_channels * (i + 1))], child_code.detach()))
                l1loss +=  decrease_ratio * this_l1loss_list[i]
                cosloss +=  decrease_ratio * cor_square_error(recon_child_code[:, (args.MODEL.mapping.out_channels * i):(args.MODEL.mapping.out_channels * (i + 1))], child_code.detach())
            ...
            if args.TRAIN.inference and idx%args.TRAIN.save_iter==0:
                with torch.no_grad():
                    recon_img  = G_net.synthesize(recon_child_code)
                    encode_img = G_net.synthesize(child_code.detach())
