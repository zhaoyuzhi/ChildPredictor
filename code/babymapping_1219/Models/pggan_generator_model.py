...
_RESOLUTIONS_TO_CHANNELS = {
    ...
    128: [512, 512, 512, 512, 512, 256, 128], #2x2 4x4 8x8 16x16 32x32 64x64 128x128
...
_PGGAN_PTH_VARS_TO_TF_VARS = {
    ...
    'layer11.conv.weight': '128x128/Conv1/weight',  # [128, 128, 3, 3]
    'layer11.wscale.bias': '128x128/Conv1/bias',  # [128]
    'output0.conv.weight': 'ToRGB_lod5/weight',  # [3, 512, 1, 1]
    'output0.wscale.bias': 'ToRGB_lod5/bias',  # [3]
    'output1.conv.weight': 'ToRGB_lod4/weight',  # [3, 512, 1, 1]
    'output1.wscale.bias': 'ToRGB_lod4/bias',  # [3]
    'output2.conv.weight': 'ToRGB_lod3/weight',  # [3, 512, 1, 1]
    'output2.wscale.bias': 'ToRGB_lod3/bias',  # [3]
    'output3.conv.weight': 'ToRGB_lod2/weight',  # [3, 512, 1, 1]
    'output3.wscale.bias': 'ToRGB_lod2/bias',  # [3]
    'output4.conv.weight': 'ToRGB_lod1/weight',  # [3, 256, 1, 1]
    'output4.wscale.bias': 'ToRGB_lod1/bias',  # [3]
    'output5.conv.weight': 'ToRGB_lod0/weight',  # [3, 128, 1, 1]
    'output5.wscale.bias': 'ToRGB_lod0/bias',  # [3]
}
...
"""
    ...
    'layer1.conv.weight': '4x4/Conv/weight',  # [512, 512, 3, 3]
    'layer1.wscale.bias': '4x4/Conv/bias',  # [512]
    'layer2.conv.weight': '8x8/Conv0_up/weight',  # [512, 512, 3, 3]
    'layer2.wscale.bias': '8x8/Conv0_up/bias',  # [512]
    'layer3.conv.weight': '8x8/Conv1/weight',  # [512, 512, 3, 3]
    'layer3.wscale.bias': '8x8/Conv1/bias',  # [512]
    'layer4.conv.weight': '16x16/Conv0_up/weight',  # [512, 512, 3, 3]
    'layer4.wscale.bias': '16x16/Conv0_up/bias',  # [512]
    'layer5.conv.weight': '16x16/Conv1/weight',  # [512, 512, 3, 3]
    'layer5.wscale.bias': '16x16/Conv1/bias',  # [512]
    'layer6.conv.weight': '32x32/Conv0_up/weight',  # [512, 512, 3, 3]
    'layer6.wscale.bias': '32x32/Conv0_up/bias',  # [512]
    'layer7.conv.weight': '32x32/Conv1/weight',  # [512, 512, 3, 3]
    'layer7.wscale.bias': '32x32/Conv1/bias',  # [512]
    'layer8.conv.weight': '64x64/Conv0_up/weight',  # [256, 512, 3, 3]
    'layer8.wscale.bias': '64x64/Conv0_up/bias',  # [256]
    'layer9.conv.weight': '64x64/Conv1/weight',  # [256, 256, 3, 3]
    'layer9.wscale.bias': '64x64/Conv1/bias',  # [256]
    'layer10.conv.weight': '128x128/Conv0_up/weight',  # [128, 256, 3, 3]
    'layer10.wscale.bias': '128x128/Conv0_up/bias',  # [128]
    'layer11.conv.weight': '128x128/Conv1/weight',  # [128, 128, 3, 3]
    'layer11.wscale.bias': '128x128/Conv1/bias',  # [128]
    'output0.conv.weight': 'ToRGB_lod5/weight',  # [3, 512, 1, 1]
    'output0.wscale.bias': 'ToRGB_lod5/bias',  # [3]
...
class PGGANGeneratorModel(nn.Module):
  ...
  def __init__(self,
    ...
    assert len(self.channels) == int(np.log2(resolution))  #确保每个分辨率都有对应的channals

    self.resolution = resolution
    self.fused_scale = fused_scale
    self.output_channels = output_channels

    for block_idx in range(1, len(self.channels)):#1 2 3 4 5 6 7 8
      if block_idx == 1:
        self.add_module(
            f'layer{2 * block_idx - 2}',   #layer0
        ...
        self.add_module(
            f'layer{2 * block_idx - 2}',  #      layer2  layer4  layer6  layer8  layer10 放大两倍
            ConvBlock(in_channels=self.channels[block_idx - 1],
                      out_channels=self.channels[block_idx],
                      upsample=True,
                      fused_scale=self.fused_scale))
      self.add_module(
          f'layer{2 * block_idx - 1}',  #layer1  layer3  layer5  layer7  layer9   layer11
          ConvBlock(in_channels=self.channels[block_idx],
                    out_channels=self.channels[block_idx]))
      self.add_module(
          f'output{block_idx - 1}', #output0 output1  output2  output3  output4  output5
          ConvBlock(in_channels=self.channels[block_idx],
    ...
    self.upsample = ResolutionScalingLayer()  #使用最近邻插值放大2倍
    self.lod = nn.Parameter(torch.zeros(()))  

    self.pth_to_tf_var_mapping = {}
    for pth_var_name, tf_var_name in _PGGAN_PTH_VARS_TO_TF_VARS.items():
      if self.fused_scale and 'Conv0' in tf_var_name: #no need to execute
        pth_var_name = pth_var_name.replace('conv.weight', 'weight')
        tf_var_name = tf_var_name.replace('Conv0', 'Conv0_up')
      self.pth_to_tf_var_mapping[pth_var_name] = tf_var_name  #针对设定的分辨率，挑出了对应名称

  def forward(self, x):
    if len(x.shape) != 2:
      raise ValueError(f'The input tensor should be with shape [batch_size, '
                       f'noise_dim], but {x.shape} received!')
    x = x.view(x.shape[0], x.shape[1], 1, 1)  #reshape to [batchsize, latent_dim, 1, 1]

    lod = self.lod.cpu().tolist() #0
    for block_idx in range(1, len(self.channels)):#1 2 3 4 5 6 7 8
      if block_idx + lod < len(self.channels):
        x = self.__getattr__(f'layer{2 * block_idx - 2}')(x) #layer0 layer2 layer4 layer6 layer8 layer10
        x = self.__getattr__(f'layer{2 * block_idx - 1}')(x) #layer1 layer3 layer5 layer7 layer9 layer11
        image = self.__getattr__(f'output{block_idx - 1}')(x) #output0 output1 output2 output3 output4 output5
      else:
        image = self.upsample(image)
    return image #output5  ->  128x128
