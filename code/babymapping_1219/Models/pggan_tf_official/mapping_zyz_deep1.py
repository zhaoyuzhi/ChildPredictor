...
class MappingNet(nn.Module):
    def __init__(self, in_channels, out_channels, out_num, input_norm=False, output_norm=True):
        ...
        self.sigact = nn.LeakyReLU(0.2, inplace = False) #nn.Sigmoid()
        self.final1 = nn.Linear(in_channels, out_channels * out_num // 2) #in_channels=512, out_channels = 480
        self.final2 = nn.Linear(out_channels * out_num // 2, out_channels*out_num)
        self.final  = nn.Linear(out_channels * out_num, out_channels*out_num)
    ...
    def forward(self, x_father, x_mother):
        ...
        out = self.sigact(self.final1(out))
        out = self.sigact(self.final2(out))
        out = self.final(out)
