        ...
        for face_id in range(len(combine_path)):
            ...
            if faceidx == 4:
              #  assert len(child_img_list) == 1
                ...
                child_img_list.append(img)
                child_label.append(self.get_label(img_path))
            if face_id == len(combine_path)-1 and len(child_img_list)==1:
                img_aug = child_img_list[0]
                img_aug = torchvision.transforms.functional.to_pil_image(img_aug)
                img_aug = torchvision.transforms.functional.hflip(img_aug)
                img_aug = torchvision.transforms.functional.to_tensor(img_aug)
