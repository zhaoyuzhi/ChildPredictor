...
class mapping_val(BaseData):
    def __init__(self, args):
        super(mapping_val, self).__init__(args)
        self._op_init_param(args.dataset)      #After this, all parameters defined in yaml can be used.
        self.father_dir = args.father_dir
        self.mother_dir = args.mother_dir
    ...
    def __getitem__(self, idx):
        ...
        father_img = self._op_readasTensor(self.father_dir)        #Read as a tensor, CxHxW, value range:0.0-255.0
        father_img = self._op_image_normalize(father_img)                     #normalize to -1,1

        mother_img = self._op_readasTensor(self.mother_dir)        #Read as a tensor, CxHxW, value range:0.0-255.0
        mother_img = self._op_image_normalize(mother_img)                     #normalize to -1,1
        
        return father_img, mother_img       #img shape:[C,H,W], value:-1~1, imglabel shape:[4] value:0~1
