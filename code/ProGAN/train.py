...
import config_mixture as config
import tfutil
import dataset
import misc
import pdb
...
def setup_snapshot_image_grid(G, training_set,
    ...
    uniforms = misc.random_uniforms(gw * gh, G)
    return (gw, gh), reals, labels, latents, uniforms
...
def train_progressive_gan(
    ...
    #---------------- get training_set --------------------
    training_set = dataset.load_dataset(data_dir=config.data_dir, verbose=True, **config.dataset)#返回一个对象，可以调用方法来迭代的读取数据

    #---------------Construct networks.构建网络-------------------
    with tf.device('/gpu:0'):
        if resume_run_id is not None and False:#如果有ｃｋｐ可以加载的话
            network_pkl = misc.locate_network_pkl(resume_run_id, resume_snapshot)
            print('Loading networks from "%s"...' % network_pkl)
            G, D, Gs = misc.load_pkl(network_pkl)
        else:
            print('Constructing networks...')   #构建网络，G_paper与D_paper.channel和resolution都由数据集所决定
            ...
            if resume_run_id is not None:
                import pickle
                print('finetune model based on {}'.format(resume_run_id))
                f = open(resume_run_id, 'rb')
                G_ckp, D_ckp, _ = pickle.load(f)
                G_vars = dict(G_ckp.__getstate__()['variables'])
                D_vars = dict(D_ckp.__getstate__()['variables'])
                for name in G_vars:
                    if name not in G_ex:
                        G.set_var(name, G_vars[name])
                for name in D_vars:
                    if name not in D_ex:
                        D.set_var(name, D_vars[name])
            Gs = G.clone('Gs')
    ...
    with tf.name_scope('Inputs'):
        lod_in          = tf.placeholder(tf.float32, name='lod_in', shape=[])    #Inputs中的lod_in占位
        lrate_in        = tf.placeholder(tf.float32, name='lrate_in', shape=[])  #Inputs中的lrate_in占位
        minibatch_in    = tf.placeholder(tf.int32, name='minibatch_in', shape=[])  #Inputs中的minibatch_in占位
        minibatch_split = minibatch_in // config.num_gpus#多个ｇｐｕ的话，要将一个ｂａｔｃｈ分开
        reals, labels   = training_set.get_minibatch_tf()#从ｄａｔａｓｅｔ中拿到对应数量的ｒｅａｌ图片
        reals_split     = tf.split(reals, config.num_gpus)#将ｒｅａｌ图片分成ｇｐｕｓ份
    ...
    grid_size, grid_reals, grid_labels, grid_latents, grid_uniforms = setup_snapshot_image_grid(G, training_set, **config.grid)
    sched = TrainingSchedule(total_kimg * 1000, training_set, **config.sched)
    grid_fakes = Gs.run(grid_latents, grid_labels, grid_uniforms, minibatch_size=sched.minibatch//config.num_gpus)
