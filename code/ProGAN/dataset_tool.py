...
def create_from_images(tfrecord_dir, image_dir, shuffle):
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        ...
        for idx in range(order.size):
            img = np.asarray(PIL.Image.open(image_filenames[order[idx]]))
            name = image_filenames[order[idx]].split('/')[-1].split('-')[-1] #01222021.png
            #label_value = np.zeros((1,4), dtype=np.float32)
            #0:gender 1:eyeglasses(0 for other, 1 for black) 3:smile 4:age
            label_value[order[idx],0]=0.0 if int(name[2])==2 else 1.0#0 for woman
            label_value[order[idx],1]=0.0 if int(name[6])==2 else 1.0#0 for no glass
            label_value[order[idx],2]=1.0 if int(name[5])==2 else 0.0#1 for smile
            label_value[order[idx],3]=1.0 if int(name[4])==1 else 0.0#1 for age(baby)
