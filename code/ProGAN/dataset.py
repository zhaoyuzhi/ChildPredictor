...
class TFRecordDataset:
    ...
    def __init__(self,
        ...
        tfr_shapes = [] #得到了各个文件中image的shape
        for tfr_file in tfr_files:#遍历每一个tfrecord文件
        ...
        max_shape = max(tfr_shapes, key=lambda shape: np.prod(shape))#(3,128,128)
        self.resolution = resolution if resolution is not None else max_shape[1]#128
        self.resolution_log2 = int(np.log2(self.resolution))
        self.shape = [max_shape[0], self.resolution, self.resolution]
        tfr_lods = [self.resolution_log2 - int(np.log2(shape[1])) for shape in tfr_shapes]#[5,4,3,2,1,0]
        ...
        self.label_size = self._np_labels.shape[1]#0
        self.label_dtype = self._np_labels.dtype.name#float32

        # Build TF expressions.
        with tf.name_scope('Dataset'), tf.device('/cpu:0'):
            self._tf_minibatch_in = tf.placeholder(tf.int64, name='minibatch_in', shape=[])#占位,minibatch_in
...
def load_dataset(class_name='dataset.TFRecordDataset', data_dir=None, verbose=False, **kwargs):
    ...
    dataset = tfutil.import_obj(class_name)(**adjusted_kwargs)  #args: './dataset/celebahq/'
    if verbose:
