###########################################################
#　加入了CelebA,与baby数据集中父母部分进行了混合
# 0715.在return labal时直接return fake_label，　fake_labal的方式为1-real_label，增大差异化,更利于disentangle
###########################################################
import os
import glob
import imageio
...
class attngan(BaseData):
    ...
    def __getitem__(self, idx):
        if idx < len(self.file_list):
            ...
            attr = torch.from_numpy(attr).float()
            attr_fake = 1 - attr
            return img, attr, attr_fake       #img shape:[C,H,W], value:-1~1, imglabel shape:[4] value:0~1
            ...
            imglabel_father_fake = 1 - imglabel_father
 
            return img_father, imglabel_father, imglabel_father_fake       #img shape:[C,H,W], value:-1~1, imglabel shape:[4] value:0~1
