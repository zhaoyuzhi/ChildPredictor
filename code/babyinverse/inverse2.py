...
def train(args, logger=None, vis_logger=None):
    ...
    encoder_net = Model.Vgg16(pre_train=args.MODEL.vgg.pre_train,
                              ...
                              output_dim=args.MODEL.vgg.output_dim)
    G_net = Model_PGGAN.PGGANGenerator(model_name='pggan_baby', logger=logger)
    D_net = Model.Discriminator(size=128)
    if args.USE_GPU:
    ...
    for epoch in range(0, args.TRAIN.epochs):
        ...
        for idx, (input_img, input_label) in enumerate(bar):
            ...
            init_G_code = torch.randn((input_img.shape[0], args.TRAIN.guassian_dim)).type_as(input_img)
            uniform_code = torch.Tensor(input_img.shape[0], args.TRAIN.uniform_dim).uniform_().type_as(input_img)
            input_label = input_label.cuda()
            init_G_code = init_G_code.cuda()
            uniform_code = uniform_code.cuda()

            #  init_G_code  +  label  +  uniform_code
            #       |         |
            #       |       PGGAN
            #       |         |
            #       |      init_img  <--------------------------------------
            ...
            #       |     encoder_net                                      |
            #       |         |                                            |
            #       |-->  recon_G_code    +  [label  +  uniform_code]      |
            #                                  |                           |
            #                                 PGGAN                        |
            #                                  |                           |
            #                              recon_img    <------------------|
            #print('init_G_code{}'.format(init_G_code.shape))
            #print('input_label{}'.format(input_label.shape))
            #print('uniform_code{}'.format(uniform_code.shape))
            init_code = torch.cat((init_G_code, input_label, uniform_code), dim=1)  # -> [batchsize, 512]
            #print('code{}'.format(init_code.shape))
            init_img = G_net.synthesize(init_code)
            recon_G_code,_ = encoder_net(init_img)
            recon_code = torch.cat((recon_G_code, input_label, uniform_code), dim=1)
            ...
            latent_loss = 1. * cor_square_error(recon_G_code, init_G_code) +\
                          1. * MSE_loss(recon_G_code, init_G_code)
            mse_loss = MSE_loss(recon_img, init_img)
            vgg_loss = VGG_loss(recon_img, init_img)
